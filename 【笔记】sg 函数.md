...
- SG 函数的定义说的是**单个组合游戏**。当前的状态对应于图上的一个点 $x$，那么 $x$ 所能一步到达的点的集合为 $F(x)$，则：

$$
g(x)=mex \{ g(v)\ |\ v\in F(x)\}
$$
...
$$
sg(x) = mex \{sg(l)\oplus sg(r) \ |\ l+r<x \}
$$
...
题意：有 $n$ 堆石子，第 $i$ 堆石子有 $a_i$ 个。两个人轮流取石子，每一步只能在一堆当中取 $x$ 个石子，满足 $1 \le x \le\dfrac{a_i+1}{2}$。然后 $a_i \rightarrow a_i-x$。判断先手必胜还是必败。
...
思路：做完这道题，加深了对 “利用 ${mex}$ 计算 SG 函数的过程“ 的理解。
...
对于第一个存在限制的点 $x$，假设它不能转移到当中最小的点是 $y$，那么根据 ${mex}$ 函数的定义，一定有 $sg(x)=sg(y)$。但是对于后面的点肯定不满足这个性质，因为现在的点 $x$ 虽然不能转移到 $y$，但是可能有其他 $x$ 可以转移到的点 $z$，满足 $sg(z)=sg(y)$。
